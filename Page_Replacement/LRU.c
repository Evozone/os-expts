// Write a program to demonstrate concept of LRU page replacement policy.
...
void LRU(char *refString, int numFrames, char **matrix) {
    ...
    for (int i = 0; i < strlen(refString); i++) {
        ...
        // Iterate through the refString to find the least recently used character
        int lru = 0, ageOfLRU = 0;
        // For each frame, check the age of the character
        for (int j = 0; j < numFrames; j++) {
            // Calculate the age of the character by checking number of last occurrence
            int age = 0;
            // If there is a blank character, set it to be replaced.
            if (matrix[j][i] == ' ') {
                lru = j;
                break;
            } else {
                ...
                if (age > ageOfLRU) {
                    ageOfLRU = age;
                    lru = j;
                    // If the age of the character is the same, check the frame number
                    if (age == ageOfLRU) {
                        if (j < lru) {
                            lru = j;
                        }
        ...
        // Replace the least recently used character with the current character
        matrix[lru][i] = refString[i];
    }
...
int main(int argc, char *argv[]) {
    ...
    case 1:
    case 2:
        printf("\nUsage: ./LRU <Reference String> <Number of Frames> \n");
        return 0;
    case 3:
        break;
    default:
    ...
    LRU(ref_string, num_frames, matrix);
